...
class VPNSocks5Proxy:
    ...
    def _pump(self, src: socket.socket, dst: socket.socket, is_upload: bool) -> None:
        try:
            while True:
    ...
    def _relay_tcp(self, client: socket.socket, remote: socket.socket) -> None:
        ...
        t = threading.Thread(target=self._pump, args=(client, remote, True), daemon=True)
        t.start()
        self._pump(remote, client, False)
        t.join()

        self._untrack_socket(client)
        self._untrack_socket(remote)
    ...
    def _relay_udp(
        ...
        self._track_socket(relay_sock)
        self._track_socket(ctrl_sock)
        ...
        try:
            ...
            self._untrack_socket(relay_sock)
            self._untrack_socket(ctrl_sock)
            relay_sock.close()
            self._udp_sess -= 1
        ...
        try:
            ...
            if client_sock is not None:
                try:
                    ...
                    pass
            self._active -= 1
            self._mark_disconnected(addr[0])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="VPN SOCKS5 Proxy with UDP support for Xbox NAT")
